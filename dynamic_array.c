...
static int __da_memcpy     (dynamic_array *d, size_t i, const da_data *src, size_t buffsz);
static int __da_shift_left (dynamic_array *d, size_t start, size_t amount);
static int __da_shift_right(dynamic_array *d, size_t start, size_t amount);
static size_t __da_p2_ceil (size_t n);
...
int __da_memcpy(dynamic_array *d, size_t i, const da_data *src, size_t buffsz) {
...
int __da_shift_left(dynamic_array *d, size_t start, size_t amount) {
...
int __da_shift_right(dynamic_array *d, size_t start, size_t amount) {
...
int da_rm_n(dynamic_array *d, size_t i, size_t n) {
	...
	if (i >= 0 && n > 0 && i < d->size) {
		...
		da_free(d);
		fprintf(stderr, "da_rm_n: invalid indices\n");
		return EXIT_FAILURE;
	}
}
...
void da_sort(dynamic_array *d, int (*compar)(const void *a, const void *b)) {
	qsort(d->data, d->size, sizeof(da_data), compar);
}
